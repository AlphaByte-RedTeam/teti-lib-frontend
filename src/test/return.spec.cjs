...
describe("Return Book", function () {
  ...
  it('Should Return "Si Cacing dan Kotoran Kesayangannya" Book', async function () {
    ...
    const returnButton = await driver.findElement(
      By.xpath(
        "//*[contains(@class, 'chakra-button') and contains(@class, 'css-1tfznqz')]",
      ),
    );
    ...
    const toastTextValue = await toastText.getText();
    assert.strictEqual(toastTextValue, "Returned");
  });
